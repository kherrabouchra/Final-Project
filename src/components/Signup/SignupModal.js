...
import { CheckBox, CloseBtn, SubHeader } from '../Global/GlobalComponents'
      ...
      >
        <Fade in={open}>
         ...
         <NavLink onClick={() => navigate(-1)}>
          <CloseBtn  style={{ margin:" 0 0 auto auto ", position:'absolute ', left:'10px'}}/>  
          </NavLink> 
...
<path d="M6.90514 27.3815C1.05709 30.6526 1.05709 35.9832 6.90514 39.2341C13.5507 42.9494 24.4494 42.9494 31.0949 39.2341C36.9429 35.963 36.9429 30.6324 31.0949 27.3815C24.4735 23.6864 13.5748 23.6864 6.90514 27.3815Z" stroke="black" stroke-opacity="0.82" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/>
      ...
      <BlackBtn style={{width:'140px', margin:"auto "}} onClick={handleAccountType }>Next</BlackBtn>
           ...
           <div className=   {  !isCreated ? '': 'hidden'}>
            <Header id="spring-modal-title">Create account</Header>
            <TextSub id="spring-modal-description" style={{ marginTop: 16 }}>
            Already have an account? <Link onClick={ location.pathname === "/Login" && {handleClose}} 
            to ={ location.pathname !== "/Login" &&   "/Login"}>Login.    </Link>         </TextSub>
            <React.Fragment>
       ...
       <FormControl required sx={{ m: 1, width: '45ch' }} variant="outlined">
         ...
         />       </FormControl>{error && <div style={{padding:"10px  ", margin:0}} className="errmsg">{error}</div>}
     
  </Box></React.Fragment> 
  
        <BlackBtn onClick={handleSubmit} style={{padding:"10px 170px" , margin:"0 150px" }}>Sign up</BlackBtn>
   </div>   
