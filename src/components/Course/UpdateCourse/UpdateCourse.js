...
import VideoUploader from "../CreateCourse/VideoUploader";
import {
  TypeWrapper,
  TypeContainer,
  TypeCheckBox,
...
import CsvUploader from "../CreateCourse/CsvUploader"
...
const user= location.state;
  const fetchCourses = async () => {
    try {
      const course = await api.get(`/courses/get/${id}`);
  ...
  /* const handleCsvUpload = (csvUrl, lessonIndex) => {
    setLessons((prevLessons) => {
      const updatedLessons = prevLessons.map((lesson, index) => {
        if (index === lessonIndex && lesson.type === "quiz") {
          return {
            ...lesson,
            csvUrl: csvUrl,
          };
  ...
  const validateForm = () => {
    ...
    const newErrors = lessons.map((lesson, index) => {
      ...
      if (!lesson.exercise && lesson.showQuiz) {
      ...
      if (!lesson.notebookURL && lesson.showQuiz === true) {
        error.notebookURL = `Please enter a notebook URL for the quiz`;
        hasErrors = true;
      }
            ...
            {activeStep === steps.length ? (
              ...
              <>
                ...
                >
                  ...
                  <BlackBtn
                    style={{ padding: "10px 16px" }}
                    onClick={activeStep !== 0 ? handleUpdate : handleNext}
                  >
