...
const Compete = () => {
  ...
  const calculateStatus = (endDate) => {
    ...
    const daysDiff = Math.floor(timeDiff / (1000 * 3600 * 24)); // Number of days difference
  ...
  let hackathonBanner =
    "radial-gradient(153.86% 1404.68% at 92.94% -34.87%, #000000 0%, #E7F58D 0.01%, #6FE9BB 21.7%, #2F80FF 100%)";
  return (
    <>
      <Banner color={hackathonBanner}>
        <TextWrapper style={{ marginRight: "100px" }}>
          ...
          <TextSub style={{ color: "white", margin: 0 }}>
            Join hackathons to compete against other teams, build your knowledge
            and gain experience.{" "}
          </TextSub>
        </TextWrapper>
            ...
            {data &&
              data.map((hackathon, index) => (
                <>
                  ...
                  <HackathonCard
                    key={index}
                    to={`/Compete/hackathon/${hackathon.challenge}`}
                    state={user}
                  >
                    ...
                    <Grid>
                      <Badge
                        variant="bordered"
                        color={
                          calculateStatus(hackathon.endDate) === "open"
                            ? "success"
                            : calculateStatus(hackathon.endDate) === "closed"
                            ? "error"
                            : calculateStatus(hackathon.endDate) === "upcoming"
                            ? "warning"
                            : ""
                        }
                      >
                        {calculateStatus(hackathon.endDate)}
                      </Badge>
                    </Grid>
                    <P>
                      {" "}
                      {hackathon.description <= 160
                    ...
                    >
                      <P>
                        {" "}
                        <CalendarIcon /> Ends:{" "}
                        {convertDateTimeToDate(hackathon.endDate)}/
                      </P>
                      <Arrow style={{ width: "30px", height: "30Px" }} />
                    </div>
                  </HackathonCard>{" "}
                </>
              ))}
