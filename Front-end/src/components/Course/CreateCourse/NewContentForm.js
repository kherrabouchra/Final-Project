...
const NewContentForm = () => {
  ...
  useEffect(()=>{
    ...
    api.get(`/instructor/${course.instructorID}`)
    .then((res)=>{
  ...
  return (
    <div style={{ background: "white", height: "100%" }}>
      ...
      <Link to="/instdash/courses" state={user}>
      ...
      <div style={{display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:"80px"}}>
       ...
       <CheckBox></CheckBox>
       <WhiteBtn onClick={()=>navigate ('/instdash/courses' , {state: user})}> <MdArrowBack/> Go back to courses</WhiteBtn>
       </div></>}
              </>
            )}
